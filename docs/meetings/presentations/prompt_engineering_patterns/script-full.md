...
I'm going to talk this morning about AI prompting techniques. If you have been following AI news, blogs, prompting guides, et cetera for the last few months, you may already be familiar with some of these techniques. But hopefully, there will still be something useful in here. There are more of these than I have time to cover this morning, so I will just cover a few of my favorites. You can find a full write-up with explanations and examples in Confluence if you want some more. There several more of these techniques in that guide. Alternatively, if you have some techniques that you use that are not on this list, please share them. They're probably useful to others as well. The link to those write-ups will be in the slide deck, but if you have trouble getting to it, let me know. 

I think of AI Models as if they are a team of very well-educated interns who work quickly and almost for free but they're all very naive, they're a little forgetful and they are inexperienced. Could you take a team like that and hand them instructions so that they could pause plausibly do a good job with the task? If you think you could do that, then you should be able to hand those same instructions to an AI. But the AI does need that same amount of instruction. The context of our company, the audience involved, and what sensitivity we have to have to those things are all things that we need to educate the AI in. If those aren't part of your prompt or your context, then the AI is going to struggle to do the task you are giving it. 
...
Once you have a high-quality prompt, the next step is to control how the model does its thinking. This tier of techniques provides a structure that changes how the model thinks, forcing it to be more thorough and systematic instead of jumping to a conclusion.
...
Few-shot Chain-of-Thought prompting is a way of boosting an LLM's ability to follow complex reasoning. Without specific, well-defined examples, sometimes the LLM will fall back to its parametric reasoning model and generate responses based on whatever pattern it thinks in its parametric model matches your pattern. However, if you can give it examples of high-quality output and typical input, It can use that example to find a closer match to the right pattern when it looks inside of its mathematical models. It's even more effective if you have some guides for it to use for your methodology or your reasoning. Take it through the steps that you want it to follow in the same way that you would our example team of interns when you wanted to make sure they understood how you wanted something done. If that was an important part of the process, then you would walk them through the reasoning behind it. 

The limitation of few-shot chain of thought prompting is that it puts the LLM on a specific track and it limits the comprehensiveness of its answer. If you're looking for new information, you might not get it. But if you're looking for highly structured problem-solving, such as a code refactoring problem, if you can give it good examples and a clear definition of the methodology and the techniques involved, then you can have a pretty reasonable expectation that you're going to get a better result using chain of thought prompting. 

* When to Use:
  * When you want to limit the 'creativity' of the model
  * When you have a clear definition of high quality results 
  * When you have a clearly defined methodology and standard techniques to follow
...
# **Closing**
In closing, LLMs are, at their core, pattern-recognizing machines. They're very good at transforming text and code, mapping messy inputs to structured outputs, following explicit instructions, and doing the same thing a thousand times without getting bored. They are not inherently good at making high-stakes decisions with ambiguous trade-offs the way humans do every day. Understanding organizational politics the way we do every day. Knowing context unless you give it to them or respecting boundaries you don't define. 
The right question is never, "Can AI do my job?" The question to ask yourself is, "Which parts of my job are repetitive, checkable, describable, and verifiable?" And how do I turn those into workflows that an AI can run or assist with? Do they fit the Team Of Interns scenario?
